...
using namespace std;
// Overloaded < operator that returns a boolean result if the the destination on
// the left handed side it less that the destination on the rhs side
...
// Overloaded operator that returns a boolean result if the two destinations and
// origins are equal.
bool flightRec::operator==(const flightRec &rhs) const {
